 ...
 */ 
        ...
        const unitPrice = 3600;
        let quantity = 1;

        function updatePrices() {
            const total = unitPrice * quantity;
            document.getElementById('quantity').textContent = quantity;
            document.getElementById('productPrice').textContent = '$' + unitPrice.toLocaleString('es-CO');
        ...
        function removeProduct() {
            if (confirm('¿Estás seguro de eliminar este producto del carrito?')) {
                document.querySelector('.product-card').style.animation = 'slideOutLeft 0.5s ease-out forwards';
                setTimeout(() => {
                    quantity = 0;
                    updatePrices();
        ...
        function processPayment() {
            if (quantity > 0) {
                alert('Procesando pago de $' + (unitPrice * quantity).toLocaleString('es-CO') + ' pesos...');
            } else {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const bodyPath = '/frontend/public/views/components/shopping.html';
