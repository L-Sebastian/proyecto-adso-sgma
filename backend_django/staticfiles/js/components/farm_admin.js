document.addEventListener('DOMContentLoaded', function () {
    ...
    fetch('/static/views/components/farm_admin.html')
        .then(function (res) {
...
function fn2RenderNuevas(grid) {
    ...
    grid.innerHTML = nuevas.length
        ...
        : '<p style="text-align:center;color:#666;padding:24px;grid-column:1/-1;font-size:16px;">No hay fincas nuevas recientes.</p>';
}
...
function fn2RenderMias(grid) {
    ...
    grid.innerHTML = mias.length
        ...
        : '<p style="text-align:center;color:#666;padding:24px;grid-column:1/-1;font-size:16px;">Aún no tienes fincas registradas.</p>';
}
...
function fn2HandleClick(e, gridNuevas, gridMias) {
    ...
    if (btnDelete) {
        ...
        e.stopPropagation();
        window.location.href = '/static/views/views_edit_farm.html?id=' + btnEdit.dataset.id;
        ...
        window.location.href = '/static/views/views_farm_details.html?id=' + id;
    }
}
