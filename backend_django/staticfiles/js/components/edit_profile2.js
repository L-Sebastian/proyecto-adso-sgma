document.addEventListener('DOMContentLoaded', function () {
    ...
    Promise.all([
        fetch('/static/views/components/edit_profile2.html').then(function (r) {
            if (!r.ok) throw new Error('Error cargando edit_profile2.html');
            return r.text();
        }),
        fetch('/static/views/components/modal_confirm.html').then(function (r) {
...
function initProfileEdit() {
    ...
    const savedPhoto = currentUser.photo || localStorage.getItem('profilePhoto');
    if (savedPhoto) {
        if (avatarImg) { 
    ...
    FIELDS.forEach(function (field) {
        const saved = currentUser[field.key];
        if (saved !== undefined) {
    ...
    function guardarDatos() {
        ...
        FIELDS.forEach(function (field) {
            const el = document.querySelector(field.cls);
            if (el) {
                updatedUser[field.key] = el.value;
            }
        });
    ...
    if (photoInput) {
        photoInput.addEventListener('change', function () {
            ...
            reader.onload = function (e) {
                ...
                // Guardar foto en usuario
                let users = JSON.parse(localStorage.getItem("users")) || [];
                const index = users.findIndex(u => u.email === currentUser.email);

                if (index !== -1) {
                    users[index].photo = dataURL;
                    localStorage.setItem("users", JSON.stringify(users));
                    localStorage.setItem("currentUser", JSON.stringify(users[index]));
                }
    ...
    if (btnVolver && modal) {
        btnVolver.addEventListener('click', function(e) {
            ...
            if (huboCambios()) {
                ...
                window.location.href = '/static/views/views_edit_profile.html';
            }
        });
    } else {
